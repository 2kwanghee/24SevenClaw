...
[1단계: Router] ─── 의도 분석
    ...
    │
    v
[2단계: Context Manager] ─── 필요한 정보만 선별 제공
    │
    ...
    │
    v
[3단계: Harness Loop] ─── 자동 교정 루프 (MAX 5회)
    │
...
[4단계: Worker] ─── 역할 분리
    │
    ├─ WRITE_CODE: fullstack + 모듈별 agent.md 규칙
    ├─ TEST_WRITER: tdd-smart-coding 방식
    ├─ CODE_REVIEW: ai-critique (GPT + Gemini 병렬)
    └─ SECURITY_REVIEW: OWASP Top 10 체크리스트
```
...
| 단계 | 스킬 | 설명 |
|------|------|------|
| 1 | `/harness-router` | 의도 분석 + 라우팅 |
| 2 | `/harness-context` | 컨텍스트 로딩 프로토콜 |
| 3 | `/harness-loop` | 자동 교정 루프 |
| 4 | `/harness-worker` | 역할 분리 실행 |
