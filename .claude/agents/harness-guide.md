...
[4단계: Worker / 역할별 모델] ─── 역할 분리
    ...
    ├─ CODE_REVIEW: 구현과 분리된 claude -p 리뷰 세션 + code-reviewer 서브에이전트  [Sonnet]
...
| 0↳ | `metaprompt` | sonnet | 관측형 사전 정제 — 거친 태스크를 구현 스펙으로 정제 (구현 스펙 작성 방법론) |
...
- `run_claude_review()`(구현과 분리된 claude -p 읽기전용 세션, CE-390) + `code-reviewer` 서브에이전트 → CODE_REVIEW 역할의 코드 리뷰 (필요 시 2인 정족수)
- `fullstack` → WRITE_CODE 역할의 시니어 엔지니어 페르소나
- `dev-skills.md/run-tests` → 모듈별 자동 검증 명령어
- `load-recent-changes.sh` → 세션 시작 시 컨텍스트 주입
- `pre_merge_gate.py` → 자동 파이프라인 머지 직전 거버넌스 게이트(SSOT): contract-drift/ticket-ref/plan-trace 검증 + HIGH-tier 위험강등(직접머지→PR). 토글 `FLOWOPS_GOVERNANCE*` (`docs/pipeline-guide.md` Step 5.5)
