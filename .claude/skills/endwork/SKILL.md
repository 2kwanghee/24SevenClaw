...
> PjPlan.md의 일자별 계획을 참고하여 작성할 것.

(다음 작업일의 TODO를 PjPlan.md에서 참조하여 여기에 작성)
```

- 다음 작업일 날짜와 Day 번호는 PjPlan.md의 일자별 계획에서 자동으로 가져온다
- 작업 제목도 PjPlan.md에서 참조한다

### Step 4: PjPlan.md 업데이트

PjPlan.md에서 오늘 날짜에 해당하는 행의 상태를 `✅`로 변경한다.

### Step 5: Git 커밋

```bash
git add TODO.md docs/daily/ PjPlan.md
...
- 한국어로 보고한다
- PjPlan.md의 계획 변경 이력에 기록하지 않는다 (상태 업데이트는 변경이 아님)
