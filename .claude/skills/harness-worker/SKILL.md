...
**기반**: `run_codex_review.sh` (codex CLI 구독형) + `code-reviewer` 서브에이전트 (필요 시 2인 정족수)

**시스템 규칙**:
- 코드를 수정하지 않는다. 리뷰 피드백만 제공한다.
- 작성자(WRITE_CODE)와 완전히 분리된 관점에서 검토한다.
- codex CLI(별도 모델)와 독립 리뷰어 서브에이전트를 활용하여 작성자의 편향을 보완한다.
...
2. `run_codex_review.sh` 또는 `code-reviewer` 서브에이전트 호출 (변경된 코드 대상)
3. 공통 지적사항 → 반드시 수정
