...
**파싱 방법:**
1. PRD의 `##` 헤더를 기능 영역으로 분류
2. 각 영역 내 `-` 또는 `- [ ]` 항목을 개별 태스크 후보로 추출
3. 표(테이블)의 각 행도 태스크 후보로 추출
4. 코드 블록(```` ``` ````)은 구현 참조로만 사용 (태스크로 분해하지 않음)

**분해 기준:**
- 독립적으로 구현/테스트 가능한 단위
- 하나의 PR로 리뷰 가능한 크기
- 명확한 완료 조건(Done criteria)이 있을 것
- 모듈 태그 포함: `[api]`, `[web]`, `[agent]`, `[contracts]`, `[infra]`
...
사용자 확인 후, 기존 Linear 이슈와 중복 확인을 먼저 수행한다:

```bash
# 중복 확인: 기존 이슈 중 유사 제목이 있는지 검색
python3 scripts/linear_tracker.py list --status "Todo" --status "Queued" --status "In progress"
```

중복이 있으면 사용자에게 알리고, 중복되지 않는 태스크만 등록한다:
