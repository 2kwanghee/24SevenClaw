...
@pytest.mark.asyncio
async def test_full_wizard_flow(client: AsyncClient) -> None:
    """회원가입 → 프로젝트 생성 → 추천 → 설정 저장 → 프리뷰 → ZIP 다운로드 전체 흐름."""
    headers, _ = await _register_and_login(client)
    project_id = await _create_project(client, headers)

    # Step 2: 추천 엔진 호출
    rec_resp = await client.post(
        "/api/v1/recommend", json={"solution_type": "fullstack"}
    )
    assert rec_resp.status_code == 200
    rec = rec_resp.json()
    assert rec["solution_type"] == "fullstack"
    assert len(rec["agents"]) > 0
    assert len(rec["skills"]) > 0
    assert len(rec["pipelines"]) > 0

    # 위저드 데이터 구성
    wizard_data = _build_wizard_request(
        agents=["backend", "frontend", "uiux"],
        skills=["tdd", "ai-critique"],
        pipelines=["harness", "tdd", "lint-gate"],
        env_vars={"ANTHROPIC_API_KEY": "sk-ant-test-key", "GITHUB_TOKEN": "ghp_test"},
    )

    # Step 7a: 프리뷰 조회
    preview_resp = await client.post(
        f"/api/v1/projects/{project_id}/preview",
        json={k: v for k, v in wizard_data.items() if k != "env_vars"},
        headers=headers,
    )
    assert preview_resp.status_code == 200
    preview = preview_resp.json()
    assert len(preview["file_tree"]) > 0
    assert len(preview["files"]) > 0

    # 프리뷰에 CLAUDE.md 포함 확인
    assert "CLAUDE.md" in preview["files"]

    # 위저드 설정 저장 (WizardData 스키마는 agents/skills/pipelines를 list[dict]으로 기대)
    config_data = {
        "wizard_data": {
            "organization": wizard_data["organization"],
            "solution": wizard_data["solution"],
            "agents": [{"id": a} for a in wizard_data["agents"]],
            "skills": [{"id": s} for s in wizard_data["skills"]],
            "pipelines": [{"id": p} for p in wizard_data["pipelines"]],
            "platform": wizard_data["platform"],
        }
    }
    ...
    saved = get_config.json()["wizard_data"]
    assert saved["platform"]["platformId"] == "claude-code"
    agent_ids_saved = [a["id"] for a in saved["agents"]]
    assert "backend" in agent_ids_saved
    ...
    assert "my-project.zip" in gen_resp.headers["content-disposition"]

    # ZIP 검증
    with _open_zip(gen_resp.content) as zf:
        names = zf.namelist()
        ...
        assert ".claude/agents/uiux-agent.md" in names
        assert ".claude/agents/harness-guide.md" in names  # 필수 에이전트
        ...
        assert "GITHUB_TOKEN=ghp_test" in env_content
...
async def test_recommend_returns_valid_catalog_items(
    client: AsyncClient, solution_type: str
) -> None:
    """모든 솔루션 유형의 추천이 유효한 카탈로그 항목을 반환."""
    resp = await client.post(
...
@pytest.mark.asyncio
async def test_recommend_then_generate_flow(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
    """추천 결과를 ZIP 생성에 반영하는 흐름 검증."""
    # 추천 요청
    rec_resp = await client.post(
    ...
    # 추천된 에이전트/스킬 ID 추출
    rec_agent_ids = [a["id"] for a in rec["agents"]]
    rec_skill_ids = [s["id"] for s in rec["skills"]]
    rec_pipeline_ids = [p["id"] for p in rec["pipelines"]]

    # 프로젝트 생성 및 추천 기반 ZIP 생성
    project_id = await _create_project(client, auth_headers, "추천 통합 테스트")

    # 에이전트 ID 매핑: 카탈로그 agents.json ID → 엔진 agent ID
    # 엔진은 backend/frontend/uiux/devops/fullstack/harness만 인식
    engine_agent_ids = [
        aid for aid in rec_agent_ids
