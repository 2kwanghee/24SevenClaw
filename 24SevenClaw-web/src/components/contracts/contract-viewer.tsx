...
const TYPE_COLORS: Record<string, string> = {
  settings: "bg-blue-500/10 text-blue-300 border-blue-500/20",
  skill: "bg-emerald-500/10 text-emerald-300 border-emerald-500/20",
  agent: "bg-violet-500/10 text-violet-300 border-violet-500/20",
  pipeline: "bg-amber-500/10 text-amber-300 border-amber-500/20",
};
...
export function ContractViewer({ contract }: ContractViewerProps) {
  const typeColor = TYPE_COLORS[contract.contract_type] ?? "bg-slate-500/10 text-slate-300 border-slate-500/20";
  ...
  return (
    <div className="space-y-6">
      ...
      {contract.description && (
        <div>
          <p className="mb-1 text-xs font-medium text-slate-500">설명</p>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-slate-500">
        <span className="flex items-center gap-1">
          소스: <code className="rounded bg-white/[0.05] px-1.5 py-0.5">{contract.source}</code>
        </span>
        <span className="flex items-center gap-1">
      ...
      {/* JSON 콘텐츠 */}
      <div>
        <p className="mb-2 text-xs font-medium text-slate-500">콘텐츠 (JSON)</p>
        <pre className="max-h-96 overflow-auto rounded-xl border border-white/5 bg-slate-900/80 p-4 text-xs leading-relaxed text-slate-300">
