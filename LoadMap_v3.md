...
| **P2** | YAML 제어면 계약 — ControlPlane v1 + 수신 엔드포인트(서비스 키 인증) | ✅ 완료 (2026-07-28) |
| **P3** | 구독형 전용 강제 — 게이트웨이 강제 모드(FLOWOPS_SUBSCRIPTION_ONLY) + 종량 누수 수정 | ✅ 완료 (2026-07-28) |
| **P4** | 시트 풀(등록형) + seat 원장 축 + with_seat 주입 래퍼 | ✅ 완료 (2026-07-29) |
| **P5** | 다프로젝트 실행 v1(필터+러너) — 병렬은 러너 수평 확장으로 확정 | ✅ 완료 (2026-07-29) |
| **P6** | 티켓 전량 자동 발급 — 기계수락 + 분해 배치 + 3상 발급기 | ✅ 완료 (2026-07-28) |
| **P7** | 정합성 테스트 게이트 — 완주판정 + 게이트 + verified 최종 상태 | ✅ 완료 (2026-07-28) |
| P8 | 집행면 게이트 엔진 + 룰 플러그인 | ⏸ 후속 보류 — 방식 확정(층 분리 이식, docs §8-1) |
| **P9** | 기록면 1급화(delivery_events) + 체인 대시보드 | ✅ 완료 (2026-07-29) |

Phase 정의·순서 근거의 정본은 `docs/multiproject-delivery.md` §8.
**후속 과제 F-1~F-5**(사용량 인제스트·실토큰 실증·P8·게이트 자동해석·종량 스크립트 처분)는 동 문서 §8-2.
