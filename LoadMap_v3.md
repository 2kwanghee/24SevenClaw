...
| Phase | 내용 | 상태 |
|-------|------|------|
| **P0** | 거버넌스 정책 외부화(`Policy` 주입) + `DeliveryProfile` 모델·API | 진행 중 |
| P1 | 파이프라인 저장소 중립화 (`PROJECT_DIR`/브랜치/`.ralph` 파라미터화) | 대기 |
| P2 | 집행면 — 게이트 엔진 + 룰 플러그인 + fail-closed + 감사로그 | 대기 (선행조건 有) |
| P3 | 실행면 — 샤드 배정 + worktree + cwd 역할판정 | 대기 |
| P4 | 기록면 1급화 + 제어면 DB 승격 + 정합성 린터 | 대기 |
| P5 | 리뷰 2인 정족수 강제 + 적대적 검토 역할 | 대기 |

P0·P1 완료 시 T1/T2 다프로젝트 운용 성립. T3 는 P2·P3 필수.
