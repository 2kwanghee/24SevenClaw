# ClickEye - Daily TODO

> Claude가 이 파일을 참고하여 순차적으로 개발한다.
> 작업 완료 시 `[x]` 표시. 하루 마감 시 `/endwork` 명령으로 아카이브.
> Linear 프로젝트: "LoadMap v3 — 솔루션 빌더" (24S-18 ~ 24S-43)

---

## 2026-08-04

### 완료
- [x] **CE-338** 무인 체인 점화 종단 검증 — Linear→ngrok→수신 컨테이너→Redis 큐→호스트 워커→파이프라인 전 구간 실측 관통. crontab 을 정본과 완전 일치하게 재설치
- [x] **CE-349** 웹훅 재트리거 무한 루프 차단 (PR #89) — 락 보유자 생존 판정 + 체인 상한 5회. 부수 수정 2건: `webhook-doctor.sh` 컨테이너 PID 오탐, ngrok watchdog 예약 도메인 고정
- [x] 다음 스프린트 큐 리필 — CE-350~354 등재 (전부 `Wait`)
- [x] `/docs-sync` — `run_guide.md`(3-5-1 재부팅 복구 절·3-6 2단계 선행조건·STOP-CHAIN 트러블슈팅), `multiproject-delivery.md`(§6-1 주석·변경 이력)
- [x] **풀스택 런처** `scripts/fullstack_run.sh` (PR #91) — `bash scripts/fullstack_run.sh` 한 줄로 컨테이너·API·웹·워커·ngrok 기동(콜드스타트 19초 실측). `--check`/`--stop`/`--no-*`. 테스트 13케이스
- [x] compose `migrate` 이미지 재빌드 — 리비전 058 미보유로 `exit 255` 였음(= `up -d` 만으로는 마이그레이션 미적용, `api` 컨테이너도 기동 불가 상태였다)
- [x] `.run/` 추적 해제 (PR #92) — 자동커밋이 PID 파일을 물어 main 을 분기시키던 경로 차단

### 다프로젝트 체인 — 종단 최초 성공 (2026-08-04 15:48)
- [x] 리허설 레포 등재 → 조달 → 토글 → **CE-355 완주** (고객 레포 `ralph/CE-355` push, 변경 파일 `README.md` 1개)
- [x] **CE-350** 머신 서비스 키 CLI (PR #93) — 웹 로그인 없이 발급·검증·회수
- [x] **CE-356** 워크스페이스 전용 구현 프롬프트 (PR #95) — 계획 인라인, self-repo 전제 제거
- [x] **CE-357** 메타프롬프트 정제 부활 (PR #94) — 프론트매터가 CLI 옵션으로 파싱되던 것
- [x] 활성 구성 결정: 디스패처 off + 단일 러너 automap (디스패처는 시트 무조건 필수)

### 남은 관측 (별건, 미등재)
- [ ] `Codex CLI 실행 실패 → 기본 REVIEW.md` — QA 리뷰가 폴백으로 지나감
- [ ] 워크스페이스 `hasTrustDialogAccepted` 경고 — 집행면 훅 적용에 영향 있는지 확인

### 과거 차단 항목 (전부 해소 — 기록 보존)
- [x] 1단계 시트 풀 — **불필요로 판정**. 구독 계정 1개면 단일 러너가 현행 로그인 세션을 쓴다(디스패처를 켤 때만 시트 필수)
- [x] 2단계 워크스페이스 매핑 — CE-350 CLI 로 서비스 키 발급 후 폴링 성공
- [x] 3단계 조달 — 리허설 레포(`24seven-delivery-rehearsal`)로 완료

### 다음 후보 (Linear `Wait`)
- [ ] CE-350 머신 서비스 키 발급 CLI (High — 활성 절차의 실제 블로커)
- [ ] CE-351 재부팅 내구성 (High — db·redis `RestartPolicy=no`, 2026-08-04 마운트 장애로 실증)
- [ ] CE-352 큐 신뢰성 BRPOPLPUSH + confirmer 지연 비대칭 (Medium)
- [ ] CE-353 F-1 후속 정제·분해 사용량 json 전환 (Medium)
- [ ] CE-354 F-4 잔여 제어면 YAML `gates` 직접 소비 (Low)
