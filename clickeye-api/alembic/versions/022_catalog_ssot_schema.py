...
def upgrade() -> None:
    # agents 테이블 신규 컬럼
    op.add_column(
        "agents", sa.Column("required", sa.Boolean(), nullable=False, server_default="false")
    )
    op.add_column("agents", sa.Column("output_file", sa.String(200), nullable=True))
    op.add_column(
        "agents", sa.Column("dependencies", postgresql.JSONB(), nullable=False, server_default="[]")
    )

    # skills 테이블 신규 컬럼
    op.add_column(
        "skills", sa.Column("required", sa.Boolean(), nullable=False, server_default="false")
    )
    op.add_column("skills", sa.Column("output_file", sa.String(200), nullable=True))
    op.add_column(
        "skills", sa.Column("dependencies", postgresql.JSONB(), nullable=False, server_default="[]")
    )
    op.add_column(
        "skills", sa.Column("hook_events", postgresql.JSONB(), nullable=False, server_default="[]")
    )
    op.add_column(
        "skills", sa.Column("env_vars", postgresql.JSONB(), nullable=False, server_default="[]")
    )
