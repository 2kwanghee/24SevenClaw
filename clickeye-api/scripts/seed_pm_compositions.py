        ...
        for group_key, slugs in groups.items():
            ...
            for slug in slugs:
                if slug not in available:
                    errors.append(f"{pm_slug}.{group_key}: 슬러그 '{slug}'가 레지스트리에 없음")
    return errors
...
async def seed() -> None:
    async with async_session() as db:
        ...
        for pm_slug, mapping in PM_COMPOSITIONS.items():
            pm = (
                await db.execute(select(PMProfile).where(PMProfile.slug == pm_slug))
            ).scalar_one_or_none()
            if pm is None:
            ...
            await db.execute(delete(PMComposition).where(PMComposition.pm_id == pm.id))
            ...
            await db.execute(
                update(PMProfile).where(PMProfile.id == pm.id).values(bio_long=new_bio)
            )
