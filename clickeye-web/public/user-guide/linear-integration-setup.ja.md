...
## Step 4 — プロジェクトに Linear スキルを含める

デリバリーコンソールから配信されるプロジェクトに **Linear** スキルを含めると、Linear 連携スクリプトが一緒に提供されます。

プロジェクトの `.env` ファイルに次の値が必要です:
...
- [はじめに](/guide/getting-started) — ClickEye デリバリーコンソールの基本フロー
