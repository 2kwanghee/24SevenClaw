...
### What? — 무엇이 결과로 나오나
> 딜리버리 콘솔을 통해 인게이지먼트를 설계하면, 우리의 자동화 파이프라인이 하이브리드 러너로 즉시 실행. 개발부터 배포까지 자동화됨.

### Proof — 어떻게 증명하나
> 본 파이프라인은 ClickEye 의 딜리버리 콘솔 개발에 매일 적용되며, 매 마일스톤 회귀 검증 자동화로 검증됨.
...
1. **자기 자신을 만든다** — ClickEye 제품 자체의 모든 신규 기능은 ClickEye 가 생성하는 자동화 흐름과 **동일한 시스템** 으로 만들어진다.
...
- 단일 개발자 1 인으로도 **계획·설계·구현·검증·배포** 의 전체 파이프라인을 자동화.
- 매 마일스톤 회귀 검증 자동 통과. Plan Gate + Multi-Agent + 거버넌스로 신뢰도 확보.
...
## 목표
딜리버리 콘솔 — 새 인게이지먼트 생성 및 AI Team 페이지 추가

## 분해된 작업
1. Backend: Engagement 모델 + API 엔드포인트
2. Frontend: Delivery Console 페이지 + AI Team 컴포넌트
3. Tests: 단위 테스트 + 통합 테스트

## 트레이드오프 분석
- 한 세션에 다 끝낼 수 있나? → 핵심 CRUD만 + 심화 기능은 다음 마일스톤으로 분리
- 외부 의존성? → 최소 필수만 사용, 선택사항은 feature flag로 관리
```
...
```
[가림막 원칙]
  ✅ 로딩: CLAUDE.md, 모듈별 .claude/agents/*.md, 관련 src 파일만
  ❌ 차단: 전체 코드베이스 grep, node_modules, .git, 불필요한 디렉토리
```
...
회귀 검증 R-1 ~ R-5 를 매 PR 머지 전에 자동으로 통과해야 함.

| R# | 항목 | 자동화 |
|---|---|---|
| **R-1** | OpenAPI diff | git diff `openapi.json` |
| **R-2** | 기존 기능 무영향 | git diff 검증 + 단위 테스트 |
| **R-3** | Feature flag 안전성 | Feature flag 검증 + 통합 테스트 |
| **R-4** | 데이터 마이그레이션 | Alembic upgrade/downgrade 테스트 |
| **R-5** | 통합 테스트 | pytest + vitest |
...
> 비침습성 검증은 **고객 환경 무손실 보장**의 핵심. 우리가 우리 자신의 자동화로 검증 → 딜리버리 산출물로 실행 환경에 배포.
...
| 마일스톤당 테스트 통과율 | pytest + vitest 실행 결과 |
| 회귀 검증 통과율 (R-1~R-5) | git diff 검증 + 자동 테스트 |
| 자동 PR 생성 → 머지 전환율 | GitHub Actions CI/CD 로그 |
| Linear 이슈 평균 해결 시간 | Linear API |
| harness-gate 1차 통과율 | 커밋 hook 로그 |

### 향후 확장 (로드맵)

| 확장 | 시점 | 내용 |
|---|---|---|
| **메타프롬프트 고도화** | 당월 | 관측형 사전 정제 품질 개선, LLM 기반 추천 엔진 |
| **거버넌스 고도화** | 당월 | 정책 자동화, 감사 로그, 규정 준수 검증 강화 |
| **Cloud 실행 확대** | 다음 분기 | 서버 기반 자동 실행 능력 확장 (Celery 워커 풀) |
| **GitHub Enterprise 지원** | 다음 분기 | GitHub Enterprise, GitLab 통합 |
| **Multi-tenant 강화** | Enterprise 플랜 | organization-level isolation, custom 정책, 감사 |
| **AI Review 다중화** | 지속 | Claude + Codex 합의 시스템 고도화 |
