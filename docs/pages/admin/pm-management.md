...
PM의 `pm_markdown` 필드는 딜리버리 실행 컨텍스트에 주입되는 핵심 파일이다. 관리자는 GUI 폼 외에도 **마크다운 에디터** 탭에서 원문을 직접 수정할 수 있다.
...
- **pm_markdown**: 딜리버리 실행 컨텍스트에 주입될 실제 PM 지침 파일 원문 (MD 직접 편집 탭에서 관리)
