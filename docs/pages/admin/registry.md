...
|------|------|
| `name` | 표시 이름 (예: "코드 리뷰어") |
| `slug` | 고유 식별자 — 실행 환경 내 파일명 기준 (예: `code-reviewer`) |
...
| `is_public` | 카탈로그에서 일반 사용자에게 표시 여부 |
| `tags` | 연관 기술 태그 배열 |
...
Skill은 카탈로그에서 선택되어 설정에 추가되고 `.claude/skills/` 파일로 생성되는 자동화 스킬이다.
