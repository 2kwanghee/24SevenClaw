...
| `SubscriptionSeat` | provider·Runner·credential ref·상태·cooldown | 신규 |
| `SeatUsageSample` | 실행별 token 관측·rate limit·reset·confidence | 신규 |
...
현재는 답변을 추정해 기록하지 않는다. 사용자 인터뷰 후 아래에 추가한다.
