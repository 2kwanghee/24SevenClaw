...
def read_ralph_doc(filename: str) -> str:
    """Read a document from .ralph/ directory. Returns content or empty string."""
    path = os.path.join(PROJECT_DIR, ".ralph", filename)
...
def build_pr_body(
    ...
    """Build PR body with PLAN/TASK/REVIEW + Linear link."""
    lines = []
    ...
    # 📌 PLAN (Gemini 기획)
    plan_content = read_ralph_doc("PLAN.md")
    if plan_content:
        lines.append("## 📌 PLAN (Gemini)")
        lines.append("")
        lines.append("<details>")
        lines.append("<summary>기획서 펼치기</summary>")
        lines.append("")
    ...
    # ⚙️ TASK (Claude 구현)
    task_content = read_ralph_doc("TASK.md")
    if task_content:
        lines.append("## ⚙️ TASK (Claude)")
        lines.append("")
        lines.append(task_content)
        lines.append("")

    # 🧪 REVIEW (QA 리뷰 — 구현과 분리된 세션)
    review_content = read_ralph_doc("REVIEW.md")
    if review_content:
        lines.append("## 🧪 QA Review (분리 세션)")
        lines.append("")
        lines.append("<details>")
        lines.append("<summary>QA 리뷰 펼치기</summary>")
        lines.append("")
    ...
    lines.append("---")
    lines.append("*Auto-generated by Multi-Agent pipeline (기획/정제 → Claude 구현 → QA 리뷰 분리 세션)*")
