...
PYEOF
    ...
    GEN_PROMPT="$WORK_DIR/prompt_$i.md"
    {
      ...
      echo "- **모든 티켓은 대상 저장소에 파일 변경(커밋)을 만들어야 한다.** 무인 파이프라인은"
      echo "  커밋을 산출물로 판정하므로, 계약·구조를 '확정만' 하고 파일을 남기지 않는 티켓은"
      echo "  만들지 마라(실측 CE-359: 그런 티켓이 커밋 0 으로 실패 처분됐다)."
      echo "  · 설계 판단은 그것을 사용하는 구현 티켓 description 안에 넣어라."
      echo "  · 설계를 티켓으로 남겨야 한다면, 대상 저장소에 문서 파일(ADR·스펙 md)로 **써서"
      echo "    커밋**하는 티켓으로 만들어라(그러면 산출물이 있다)."
      echo "- 선행 관계가 필요하면 depends_on 으로 실행 순서를 표현하라(설계→구현 순서는 유효)."
