...
if [ -n "$FAIL_MSGS" ]; then
  echo "{\"decision\": \"block\", \"reason\": \"[iteration ${COUNT}/${MAX_ITER}] ${FAIL_MSGS}수정하라.\"}"
  exit 0
fi
...
if [ -d "$PROJECT_DIR/clickeye-api" ]; then
  ...
  if echo "$API_LINT" | grep -qE "[0-9]+ error|Found [0-9]+"; then
    if ! echo "$API_LINT" | grep -q "All checks passed"; then
      LINT_MSGS="${LINT_MSGS}API 린트: $API_LINT. "
    fi
  fi
fi
...
if [ -d "$PROJECT_DIR/clickeye-agent" ]; then
  ...
  if echo "$AGENT_LINT" | grep -qE "[0-9]+ error|Found [0-9]+"; then
    if ! echo "$AGENT_LINT" | grep -q "All checks passed"; then
      LINT_MSGS="${LINT_MSGS}Agent 린트: $AGENT_LINT. "
    fi
  fi
fi

if [ -n "$LINT_MSGS" ]; then
  echo "{\"decision\": \"block\", \"reason\": \"[iteration ${COUNT}/${MAX_ITER}] ${LINT_MSGS}수정하라.\"}"
  exit 0
fi
